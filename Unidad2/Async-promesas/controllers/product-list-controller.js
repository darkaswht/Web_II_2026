...
const campo2 = table.dataset.campo2;
...
const crearFila = (nombre, campo2Val, id) => {
    ...
    fila.querySelector("button").addEventListener("click", (e) => {
        productService.eliminar(endpoint, e.target.dataset.id)
...
productService.listar(endpoint)
    .then((data) => {
        data.forEach((item) => {
            table.appendChild(crearFila(item.nombre, item[campo2], item.id));
        });
    })
    .catch(() => window.location.href = errorUrl);
